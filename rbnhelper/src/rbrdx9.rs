...
use hudhook::windows::Win32::Graphics::Gdi::RGNDATA;

use hudhook::hooks::DummyHwnd;
use hudhook::mh::MhHook;
use hudhook::renderer::RenderState;
use hudhook::{Hooks, ImguiRenderLoop};

use crate::{RBR_GetD3dDevice9, RBR_GetD3dWindow};
...
unsafe extern "system" fn dx9_present_impl(
    ...
    let hwnd = RenderState::setup(|| {
        ...
        HWND(RBR_GetD3dWindow() as usize as isize)
    });
...
fn get_target_addrs() -> Dx9PresentType {
    ...
    let mut device = unsafe {
        IDirect3DDevice9::from_raw(RBR_GetD3dDevice9())
    };

    let present_ptr = unsafe {
        device.vtable().Present
    };

    unsafe { mem::transmute(present_ptr) }
}

pub struct ImguiRBRDx9Hooks([MhHook; 1]);

impl ImguiRBRDx9Hooks {
...
impl Hooks for ImguiRBRDx9Hooks {
